...
INNER
sed -i -f insert_state.sed src/components/LandingPageView.tsx

sed -i 's/<div className="flex items-center gap-4">/<div className="flex items-center gap-4">\n            <button onClick={() => setIsRegisterModalOpen(true)} className="hidden sm:block px-5 py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-semibold rounded-xl transition-all">Daftar<\/button>/g' src/components/LandingPageView.tsx

cat << 'INNER2' > replace_hero_btn.sed
...
INNER2
sed -i -f replace_hero_btn.sed src/components/LandingPageView.tsx

cat << 'INNER3' > add_modal.sed
/<\/div>\n  );/i\
      {/* REGISTER MODAL */}\
      {isRegisterModalOpen && (\
...
INNER3
sed -i -f add_modal.sed src/components/LandingPageView.tsx
